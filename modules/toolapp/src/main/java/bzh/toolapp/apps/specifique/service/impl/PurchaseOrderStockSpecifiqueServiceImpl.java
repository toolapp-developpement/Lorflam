...
public class PurchaseOrderStockSpecifiqueServiceImpl extends PurchaseOrderCreateStockServiceImpl {
	...
	@Inject
	public PurchaseOrderStockSpecifiqueServiceImpl(UnitConversionService unitConversionService,
	...
	@Override
	protected StockMoveLine createProductStockMoveLine(PurchaseOrderLine purchaseOrderLine, BigDecimal qty,
		...
		if (purchaseOrderLine.getQty().compareTo(BigDecimal.ZERO) != 0) {
			companyUnitPriceUntaxed = purchaseOrderLine.getCompanyExTaxTotal().divide(purchaseOrderLine.getQty(),
		...
		if (unit != null && !unit.equals(purchaseOrderLine.getUnit())) {
			...
			priceDiscounted = unitConversionService.convert(unit, purchaseOrderLine.getUnit(), priceDiscounted,
					appBaseService.getNbDecimalDigitForUnitPrice(), product);

			companyUnitPriceUntaxed = unitConversionService.convert(unit, purchaseOrderLine.getUnit(),
					companyUnitPriceUntaxed, appBaseService.getNbDecimalDigitForUnitPrice(), product);
		}

		BigDecimal shippingCoef = shippingCoefService.getShippingCoef(product, purchaseOrder.getSupplierPartner(),
				purchaseOrder.getCompany(), qty);
		...
		return stockMoveLineSpecifiqueCreationService.createStockMoveLine(product, purchaseOrderLine.getProductName(),
				purchaseOrderLine.getDescription(), qty, BigDecimal.ZERO, priceDiscounted, companyUnitPriceUntaxed,
				companyPurchasePrice, unit, stockMove, StockMoveLineService.TYPE_PURCHASES, purchaseOrder.getInAti(),
				taxRate, null, purchaseOrderLine);
	}

	@Override
	protected StockMoveLine createTitleStockMoveLine(PurchaseOrderLine purchaseOrderLine, StockMove stockMove)
			throws AxelorException {

		return stockMoveLineSpecifiqueCreationService.createStockMoveLine(purchaseOrderLine.getProduct(),
				purchaseOrderLine.getProductName(), purchaseOrderLine.getDescription(), BigDecimal.ZERO,
				BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, null, stockMove, 2,
				purchaseOrderLine.getPurchaseOrder().getInAti(), null, null, purchaseOrderLine);
	}
}
