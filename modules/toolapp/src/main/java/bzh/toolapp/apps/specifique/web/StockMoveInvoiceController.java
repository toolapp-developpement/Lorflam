...
public class StockMoveInvoiceController {
	...
	private void sendToInvoice(ActionResponse response, List<StockMove> stockMoveList) {
		try {
			...
			Map<String, Object> mapResult = Beans.get(StockMoveMultiInvoiceService.class)
			...
			if (paymentConditionToCheck || paymentModeToCheck || contactPartnerToCheck) {
				ActionViewBuilder confirmView = ActionView.define("StockMove").model(StockMove.class.getName())
				...
				Optional<Invoice> invoice = Beans.get(StockMoveMultiInvoiceService.class)
