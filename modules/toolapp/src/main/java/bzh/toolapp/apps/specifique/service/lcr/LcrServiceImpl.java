...
public class LcrServiceImpl implements LcrService {
	...
	@Inject
	public LcrServiceImpl(BankOrderCreateService bankOrderCreateService, BankOrderLineService bankOrderLineService,
			BankOrderRepository bankOrderRepository, InvoiceRepository invoiceRepository) {
	...
	public void generateLcrFile(LocalDate dueDate, Company company) throws AxelorException {
		...
		PaymentMode paymentMode = Query.of(PaymentMode.class).filter("self.code = ?1", paymentModeCode).fetchOne();
		Company senderCompany = Query.of(Company.class).filter("self.name = ?1", companyName).fetchOne();
		List<Invoice> invoiceList = invoiceRepository.all().filter("self.dueDate <= ?1", dueDate).fetch();
		...
		for (Invoice invoice : invoiceList) {
			BankOrderLine bankOrderLine = bankOrderLineService.createBankOrderLine(paymentMode.getBankOrderFileFormat(),
					null, invoice.getPartner(), invoice.getBankDetails(), invoice.getAmountPaid(),
					invoice.getCurrency(), bankOrderDate, invoice.getId().toString(), "", invoice);
