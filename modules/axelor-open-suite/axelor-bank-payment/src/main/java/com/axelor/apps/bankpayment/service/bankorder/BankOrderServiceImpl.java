...
public class BankOrderServiceImpl implements BankOrderService {
	...
	public BankOrderServiceImpl(BankOrderRepository bankOrderRepo, InvoicePaymentRepository invoicePaymentRepo,
			...
			InvoicePaymentCancelService invoicePaymentCancelService, BankPaymentConfigService bankPaymentConfigService,
			SequenceService sequenceService, BankOrderLineOriginService bankOrderLineOriginService,
	...
	public void validatePayment(BankOrder bankOrder) throws AxelorException {
		...
		InvoicePaymentValidateServiceBankPayImpl invoicePaymentValidateServiceBankPayImpl = Beans
				.get(InvoicePaymentValidateServiceBankPayImpl.class);

		for (InvoicePayment invoicePayment : invoicePaymentList) {
			if (invoicePayment != null && invoicePayment.getStatusSelect() != InvoicePaymentRepository.STATUS_VALIDATED
	...
	public void confirm(BankOrder bankOrder)
		...
		if (Beans.get(AppBankPaymentService.class).getAppBankPayment().getEnableEbicsModule() && paymentMode != null
	...
	public void validate(BankOrder bankOrder) throws AxelorException {
		...
		if (bankPaymentConfigService.getBankPaymentConfig(bankOrder.getSenderCompany())
	...
	public void realize(BankOrder bankOrder) throws AxelorException {
		...
		if (Beans.get(AppBankPaymentService.class).getAppBankPayment().getEnableEbicsModule()) {
			...
			if (bankOrder.getSignatoryEbicsUser().getEbicsPartner().getTransportEbicsUser() == null) {
				throw new AxelorException(bankOrder.getSignatoryEbicsUser().getEbicsPartner(),
	...
	protected void sendBankOrderFile(BankOrder bankOrder) throws AxelorException {
		...
		if (bankOrder.getSignatoryEbicsUser().getEbicsPartner()
				.getEbicsTypeSelect() == EbicsPartnerRepository.EBICS_TYPE_TS) {
	...
	protected void sendFile(BankOrder bankOrder, File dataFileToSend, File signatureFileToSend) throws AxelorException {
		...
		ebicsService.sendFULRequest(signatoryEbicsUser.getEbicsPartner().getTransportEbicsUser(), signatoryEbicsUser,
				null, dataFileToSend, bankOrder.getBankOrderFileFormat(), signatureFileToSend);
	}
	...
	protected void setBankOrderSeq(BankOrder bankOrder, Sequence sequence) throws AxelorException {
		bankOrder.setBankOrderSeq((sequenceService.getSequenceNumber(sequence, bankOrder.getBankOrderDate())));

		if (bankOrder.getBankOrderSeq() != null) {
			return;
		}

		throw new AxelorException(bankOrder, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
				I18n.get(IExceptionMessage.BANK_ORDER_COMPANY_NO_SEQUENCE), bankOrder.getSenderCompany().getName());
	}
	...
	@Override
	public ActionViewBuilder buildBankOrderLineView(String gridViewName, String formViewName, String viewDomain) {
		return ActionView.define(I18n.get("Bank Order Lines")).model(BankOrderLine.class.getName())
				.add("grid", gridViewName).add("form", formViewName).domain(viewDomain);
	}
